<template>
  ...
  <div class="alert alert-warning" role="alert" v-if="filteredtipos.length===0">De <strong>{{searchTextRule}}</strong> me quedao a cero pisha</div>
  </div>
  <div class="mb-3" v-if="codigoReceived">
    <h3>Productos de <strong>{{ codigoReceived }}</strong></h3>
    <button class="btn btn-lg btn-warning" @click="resetFilter">Mostrar todos los servicios</button>
  <div class="alert alert-warning" role="alert" v-if="filteredtipos.length===0">De <strong>{{codigoReceived}}</strong> me quedao a cero pisha</div>
  </div>
      <div>
      <div class="row row-cols-1 row-cols-md-3 g-4 justify-content-center">
          <div class="col" v-for="tipo in filteredtipos" :key="tipo.codigo">
            <div class="card text-center mx-auto">
              <div class="card-body">
                <p class="card-codigo">{{tipo.codigo}} - {{tipo.tipo}}</p>
              </div>
          </div>
        </div>
      </div>
  ...
  onMounted (()=>{
    axios.get('http://127.0.0.1:8000/api/tipos/').then(
      response => {
